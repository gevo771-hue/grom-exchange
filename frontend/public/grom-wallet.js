...
/* =====================================================================
 * NAV EXTRAS + URL SYMBOL ROUTING (added 2026-06-15, rewritten v3)
 *
 ...
 *
 * 2. Two new sidebar items — "Прогнозы" (prediction markets) and "Акции"
 *    (tokenized stocks). Implemented as FULL internal GROM pages with
 *    mock market data — no redirects to Binance. Hooked into Cursor's
 *    data-page router (aside.sidebar > div.nav-item[data-page]).
 * ==================================================================== */
...
/* ---- Mock market data for the two internal pages ---- */
const GW_PREDICT_MARKETS = [
  { id: 'mlb_atl_cws', cat: 'Спорт · MLB', q: 'Atlanta Braves vs Chicago White Sox — кто победит?', a: 'Atlanta Braves', b: 'Chicago White Sox', pa: 0.49, pb: 0.51, vol: 184230, ends: '15 июн, 21:40 UTC' },
  { id: 'nl_jp_wc',    cat: 'Спорт · ЧМ-2026', q: 'Нидерланды vs Япония — Нидерланды выйдут в плей-офф?', a: 'Да', b: 'Нет', pa: 0.62, pb: 0.38, vol: 412980, ends: '22 июн, 18:00 UTC' },
  { id: 'btc_120k',    cat: 'Крипто', q: 'BTC закроет июнь выше $120K?', a: 'Да', b: 'Нет', pa: 0.34, pb: 0.66, vol: 2840110, ends: '30 июн, 23:59 UTC' },
  { id: 'eth_etf_q3',  cat: 'Крипто · Регуляция', q: 'SEC одобрит ETH staking ETF до конца Q3?', a: 'Да', b: 'Нет', pa: 0.41, pb: 0.59, vol: 1180550, ends: '30 сен, 23:59 UTC' },
  { id: 'fomc_25bps',  cat: 'Экономика · FOMC', q: 'ФРС снизит ставку на 25 bps на июльском заседании?', a: 'Да', b: 'Нет', pa: 0.71, pb: 0.29, vol: 3104000, ends: '31 июл, 18:00 UTC' },
  { id: 'sol_5b_tvl',  cat: 'Крипто · TVL', q: 'TVL Solana DeFi превысит $15B в этом квартале?', a: 'Да', b: 'Нет', pa: 0.28, pb: 0.72, vol: 624000, ends: '30 сен, 23:59 UTC' },
];

const GW_XSTOCKS = [
  { sym: 'AAPL',  name: 'Apple Inc.',          price: 234.18, chg:  0.84, vol24: '$48.2M', mc: '$3.56T'  },
  { sym: 'MSFT',  name: 'Microsoft Corp.',     price: 469.55, chg:  1.20, vol24: '$31.7M', mc: '$3.49T'  },
  { sym: 'NVDA',  name: 'NVIDIA Corp.',        price: 144.62, chg: -2.31, vol24: '$92.4M', mc: '$3.55T'  },
  { sym: 'TSLA',  name: 'Tesla Inc.',          price: 351.04, chg:  3.42, vol24: '$58.6M', mc: '$1.12T'  },
  { sym: 'GOOGL', name: 'Alphabet Inc. (C)',   price: 191.78, chg:  0.46, vol24: '$22.1M', mc: '$2.37T'  },
  { sym: 'META',  name: 'Meta Platforms',      price: 612.40, chg: -0.92, vol24: '$19.8M', mc: '$1.55T'  },
  { sym: 'AMZN',  name: 'Amazon.com Inc.',     price: 232.45, chg:  1.18, vol24: '$28.3M', mc: '$2.44T'  },
  { sym: 'AMD',   name: 'Advanced Micro Dev.', price: 134.92, chg: -1.07, vol24: '$14.6M', mc: '$218B'   },
  { sym: 'COIN',  name: 'Coinbase Global',     price: 308.50, chg:  4.62, vol24: '$11.2M', mc: '$77B'    },
  { sym: 'MSTR',  name: 'MicroStrategy',       price: 412.30, chg:  6.18, vol24: '$24.8M', mc: '$96B'    },
  { sym: 'SPY',   name: 'S&P 500 ETF',         price: 596.78, chg:  0.34, vol24: '$210M',  mc: '$598B'   },
  { sym: 'QQQ',   name: 'Nasdaq-100 ETF',      price: 524.13, chg:  0.61, vol24: '$182M',  mc: '$305B'   },
];

const GW_EXTRA_PAGES = [
  {
    page: 'predict',  label: 'Прогнозы', icon: '🎯', badge: 'BETA',
    sprite: '#i-markets',
  },
  {
    page: 'xstocks',  label: 'Акции',    icon: '📈', badge: 'NEW',
    sprite: '#i-markets',
  },
];

/* ---- Internal page HTML builders ---- */
function gwBuildPredictPage() {
  const fmt = (n) => '$' + (n >= 1e6 ? (n/1e6).toFixed(2)+'M' : n >= 1e3 ? (n/1e3).toFixed(1)+'K' : n.toFixed(0));
  const card = (m) => `
    <div class="gwx-card" data-mid="${m.id}">
      <div class="gwx-cat">${m.cat}</div>
      <div class="gwx-q">${m.q}</div>
      <div class="gwx-meta"><span>Объём 24ч · ${fmt(m.vol)}</span><span>Закрытие · ${m.ends}</span></div>
      <div class="gwx-bets">
        <button class="gwx-bet gwx-yes" data-side="a" data-mid="${m.id}">
          <span class="gwx-bet-label">${m.a}</span>
          <span class="gwx-bet-price">${(m.pa*100).toFixed(0)}¢</span>
        </button>
        <button class="gwx-bet gwx-no" data-side="b" data-mid="${m.id}">
          <span class="gwx-bet-label">${m.b}</span>
          <span class="gwx-bet-price">${(m.pb*100).toFixed(0)}¢</span>
        </button>
      </div>
    </div>`;
  return `
    <div class="gwx-wrap">
      <div class="gwx-hero">
        <div class="gwx-hero-row">
          <h1 class="gwx-h1">🎯 Prediction Markets <span class="gwx-badge gwx-beta">BETA</span></h1>
          <div class="gwx-stats">
            <div><b>$8.4M</b><small>Объём 24ч</small></div>
            <div><b>${GW_PREDICT_MARKETS.length}</b><small>Активных рынков</small></div>
            <div><b>0.5%</b><small>Комиссия</small></div>
          </div>
        </div>
        <p class="gwx-sub">Торгуй исходами реальных событий — спорт, крипто, выборы, экономика. Цена контракта = вероятность исхода. Расчёт автоматический после события.</p>
        <div class="gwx-cats">
          <button class="gwx-pill gwx-pill-on">Все</button>
          <button class="gwx-pill">Спорт</button>
          <button class="gwx-pill">Крипто</button>
          <button class="gwx-pill">Экономика</button>
          <button class="gwx-pill">Политика</button>
          <button class="gwx-pill">Культура</button>
        </div>
      </div>
      <div class="gwx-grid">${GW_PREDICT_MARKETS.map(card).join('')}</div>
      <div class="gwx-foot">
        Контракты расчитываются автоматически через oracle (Chainlink / UMA) после фиксации исхода. Комиссия GROM — 0.5% с выигрыша. <a href="#help" class="gwx-link">Подробнее →</a>
      </div>
    </div>`;
}

function gwBuildXstocksPage() {
  const row = (s) => `
    <div class="gwx-row" data-sym="${s.sym}">
      <div class="gwx-row-sym">
        <div class="gwx-tick">${s.sym}</div>
        <div class="gwx-name">${s.name}</div>
      </div>
      <div class="gwx-price">$${s.price.toFixed(2)}</div>
      <div class="gwx-chg ${s.chg >= 0 ? 'gwx-up' : 'gwx-dn'}">${s.chg >= 0 ? '+' : ''}${s.chg.toFixed(2)}%</div>
      <div class="gwx-col-vol">${s.vol24}</div>
      <div class="gwx-col-mc">${s.mc}</div>
      <div class="gwx-row-actions">
        <button class="gwx-trade gwx-buy"  data-sym="${s.sym}" data-side="buy">Купить</button>
        <button class="gwx-trade gwx-sell" data-sym="${s.sym}" data-side="sell">Продать</button>
      </div>
    </div>`;
  return `
    <div class="gwx-wrap">
      <div class="gwx-hero">
        <div class="gwx-hero-row">
          <h1 class="gwx-h1">📈 Tokenized Stocks <span class="gwx-badge gwx-new">NEW</span></h1>
          <div class="gwx-stats">
            <div><b>50+</b><small>Тикеров</small></div>
            <div><b>24/7</b><small>Торговля</small></div>
            <div><b>0.1%</b><small>Комиссия</small></div>
          </div>
        </div>
        <p class="gwx-sub">Реальные акции S&P 500 / NASDAQ обёрнутые в токены на BSC/Solana. Цена 1:1 с биржей. Эмиссия — Backed Finance / Dinari (регулируемые EU/US).</p>
        <div class="gwx-cats">
          <button class="gwx-pill gwx-pill-on">Топ</button>
          <button class="gwx-pill">Технологии</button>
          <button class="gwx-pill">Финансы</button>
          <button class="gwx-pill">Energy</button>
          <button class="gwx-pill">ETFs</button>
        </div>
      </div>
      <div class="gwx-table">
        <div class="gwx-thead">
          <div>Тикер</div><div>Цена</div><div>24ч %</div><div class="gwx-col-vol">Объём 24ч</div><div class="gwx-col-mc">Market Cap</div><div></div>
        </div>
        ${GW_XSTOCKS.map(row).join('')}
      </div>
      <div class="gwx-foot">
        Расчёт через смарт-контракты BSC / Solana. Поддержка маржин-торговли до 5x. Дивиденды распределяются автоматически держателям токенов. <a href="#help" class="gwx-link">FAQ →</a>
      </div>
    </div>`;
}

function gwBuildExtraPageHtml(page) {
  if (page === 'predict') return gwBuildPredictPage();
  if (page === 'xstocks') return gwBuildXstocksPage();
  return '';
}

/* ---- Inject scoped CSS once ---- */
function gwInjectExtraPageCss() {
  if (document.getElementById('gwExtraPageCss')) return;
  const css = `
    #gwExtraPage { position: fixed; top: 60px; left: 232px; right: 0; bottom: 0; background:#0a1224; overflow-y:auto; z-index: 30; }
    @media (max-width: 900px) { #gwExtraPage { left: 0; } }
    .gwx-wrap { padding: 28px 32px 80px; max-width: 1280px; margin: 0 auto; color: var(--silver1, #e7eef8); font-family: inherit; }
    .gwx-hero { margin-bottom: 24px; }
    .gwx-hero-row { display:flex; justify-content:space-between; align-items:center; gap:24px; flex-wrap:wrap; margin-bottom: 8px; }
    .gwx-h1 { margin:0; font-size: 28px; font-weight: 800; letter-spacing: -0.5px; display:flex; align-items:center; gap:12px; }
    .gwx-badge { font-size: 10px; font-weight: 800; letter-spacing: .8px; padding: 3px 8px; border-radius: 6px; }
    .gwx-beta { background: rgba(58,194,255,.18); color: #3ac2ff; }
    .gwx-new  { background: rgba(34,197,94,.18);  color: #22c55e; }
    .gwx-stats { display:flex; gap:24px; }
    .gwx-stats > div { display:flex; flex-direction:column; }
    .gwx-stats b { font-size: 16px; font-weight: 800; color: var(--silver1, #e7eef8); }
    .gwx-stats small { font-size: 11px; color: var(--silver5, #6b7a92); }
    .gwx-sub { color: var(--silver4, #8c9bb2); font-size: 14px; line-height: 1.55; max-width: 720px; margin: 6px 0 18px; }
    .gwx-cats { display:flex; gap:8px; flex-wrap:wrap; margin-bottom: 4px; }
    .gwx-pill { background: rgba(255,255,255,.04); color: var(--silver3,#9ba9bf); border: 1px solid rgba(255,255,255,.06); border-radius: 999px; padding: 7px 14px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all .15s; }
    .gwx-pill:hover { background: rgba(255,255,255,.07); color: var(--silver1,#e7eef8); }
    .gwx-pill-on { background: rgba(58,194,255,.18); color: #3ac2ff; border-color: rgba(58,194,255,.3); }

    /* Predict cards */
    .gwx-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px,1fr)); gap: 14px; }
    .gwx-card { background: rgba(255,255,255,.03); border: 1px solid rgba(255,255,255,.06); border-radius: 16px; padding: 18px; transition: all .15s; }
    .gwx-card:hover { background: rgba(255,255,255,.05); border-color: rgba(58,194,255,.2); transform: translateY(-1px); }
    .gwx-cat { color: var(--silver5,#6b7a92); font-size: 11px; font-weight: 700; letter-spacing: .5px; text-transform: uppercase; margin-bottom: 6px; }
    .gwx-q   { color: var(--silver1,#e7eef8); font-size: 15px; font-weight: 700; line-height: 1.4; margin-bottom: 12px; min-height: 44px; }
    .gwx-meta { display:flex; justify-content:space-between; gap:8px; color: var(--silver5,#6b7a92); font-size: 11px; margin-bottom: 12px; }
    .gwx-bets { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
    .gwx-bet { display:flex; flex-direction:column; align-items:center; gap:4px; padding: 10px; border-radius: 10px; border: 1px solid transparent; font-weight:700; cursor: pointer; transition: all .12s; }
    .gwx-yes { background: rgba(34,197,94,.10); color: #22c55e; border-color: rgba(34,197,94,.2); }
    .gwx-yes:hover { background: rgba(34,197,94,.18); }
    .gwx-no  { background: rgba(239,68,68,.10); color: #f87171; border-color: rgba(239,68,68,.2); }
    .gwx-no:hover  { background: rgba(239,68,68,.18); }
    .gwx-bet-label { font-size: 12px; font-weight: 700; }
    .gwx-bet-price { font-size: 18px; font-weight: 800; }

    /* xStocks table */
    .gwx-table { background: rgba(255,255,255,.02); border: 1px solid rgba(255,255,255,.06); border-radius: 16px; overflow: hidden; }
    .gwx-thead, .gwx-row { display: grid; grid-template-columns: minmax(180px, 2fr) 1fr 1fr 1fr 1fr auto; gap: 16px; align-items: center; padding: 14px 18px; }
    ...
    .gwx-row { border-bottom: 1px solid rgba(255,255,255,.04); transition: background .12s; }
    .gwx-row:last-child { border-bottom: 0; }
    .gwx-row:hover { background: rgba(255,255,255,.025); }
    .gwx-row-sym { display:flex; flex-direction:column; }
    .gwx-tick { font-weight: 800; font-size: 14px; }
    .gwx-name { color: var(--silver5,#6b7a92); font-size: 11px; }
    .gwx-price { font-weight: 700; font-size: 14px; }
    .gwx-chg { font-weight: 700; font-size: 13px; }
    .gwx-up { color: #22c55e; }
    .gwx-dn { color: #f87171; }
    .gwx-col-vol, .gwx-col-mc { color: var(--silver3,#9ba9bf); font-size: 13px; }
    .gwx-row-actions { display:flex; gap: 6px; justify-self:end; }
    .gwx-trade { padding: 6px 12px; border-radius: 8px; border: 0; font-weight: 700; font-size: 12px; cursor: pointer; }
    .gwx-buy  { background: rgba(34,197,94,.20); color: #22c55e; }
    .gwx-buy:hover  { background: rgba(34,197,94,.32); }
    .gwx-sell { background: rgba(239,68,68,.20); color: #f87171; }
    .gwx-sell:hover { background: rgba(239,68,68,.32); }
    @media (max-width: 1100px) {
      .gwx-thead .gwx-col-vol, .gwx-thead .gwx-col-mc,
      .gwx-row   .gwx-col-vol, .gwx-row   .gwx-col-mc { display: none; }
      .gwx-thead, .gwx-row { grid-template-columns: 1.5fr 1fr 1fr auto; }
    }

    .gwx-foot { color: var(--silver5,#6b7a92); font-size: 12px; line-height: 1.6; margin-top: 24px; padding-top: 16px; border-top: 1px solid rgba(255,255,255,.05); }
    .gwx-link { color: #3ac2ff; text-decoration: none; }
    .gwx-link:hover { text-decoration: underline; }
  `;
  const style = document.createElement('style');
  style.id = 'gwExtraPageCss';
  style.textContent = css;
  document.head.appendChild(style);
}

/* ---- Open/close overlay + sync sidebar active state ---- */
function gwShowExtraPage(page) {
  gwInjectExtraPageCss();
  ...
  mount.innerHTML = gwBuildExtraPageHtml(page);
  mount.style.display = 'block';
  mount.dataset.page = page;
  // De-activate other sidebar items, activate ours
  document.querySelectorAll('aside.sidebar .nav-item').forEach(el => {
    if (el.dataset.page === page) el.classList.add('active');
    else el.classList.remove('active');
  });
  // Sync URL (silent — don't trigger hashchange storm)
  if (location.hash !== '#' + page) {
    history.replaceState(null, '', '#' + page);
  }
  // Wire up local click handlers (toast for now — no real backend yet)
  mount.querySelectorAll('.gwx-bet').forEach(btn => {
    btn.addEventListener('click', () => {
      gwToast('Ставка на "' + btn.querySelector('.gwx-bet-label').textContent + '" — оформление откроется в следующем апдейте. Beta.');
    });
  });
  mount.querySelectorAll('.gwx-trade').forEach(btn => {
    btn.addEventListener('click', () => {
      const sym = btn.dataset.sym;
      const side = btn.dataset.side === 'buy' ? 'Купить' : 'Продать';
      gwToast(side + ' токенизированную ' + sym + ' — модалка появится в следующем апдейте. Beta.');
    });
  });
  mount.querySelectorAll('.gwx-pill').forEach(p => {
    p.addEventListener('click', () => {
      mount.querySelectorAll('.gwx-pill').forEach(x => x.classList.remove('gwx-pill-on'));
      p.classList.add('gwx-pill-on');
    });
  });
}

function gwHideExtraPage() {
  const mount = document.getElementById('gwExtraPage');
  if (mount) { mount.style.display = 'none'; mount.dataset.page = ''; }
}

function gwToast(msg) {
  let t = document.getElementById('gwToast');
  if (!t) {
    t = document.createElement('div');
    t.id = 'gwToast';
    t.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#1a2438;color:#e7eef8;padding:12px 18px;border-radius:10px;border:1px solid rgba(58,194,255,.3);font-size:13px;font-weight:600;box-shadow:0 8px 32px rgba(0,0,0,.4);z-index:100;max-width:90vw;text-align:center';
...
/* ---- Inject sidebar items + intercept clicks ---- */
function gwInjectExtraNavItems() {
  // Clean up any stray clones from earlier broken selectors (top hub-nav etc)
  document.querySelectorAll('nav.hub-nav #gwNav_predict, nav.hub-nav #gwNav_xstocks, .qa-grid #gwNav_predict, .qa-grid #gwNav_xstocks').forEach(el => el.remove());

  const aside = document.querySelector('aside.sidebar');
  if (!aside) return false;
  const items = Array.from(aside.querySelectorAll('.nav-item'));
  const markets = items.find(el => (el.textContent || '').replace(/\s+/g, ' ').trim() === 'Рынки');
  if (!markets) return false;

  for (const p of GW_EXTRA_PAGES) {
    if (aside.querySelector('#gwNav_' + p.page)) continue;
    const node = markets.cloneNode(true);
    node.id = 'gwNav_' + p.page;
    node.classList.remove('active');
    node.setAttribute('data-page', p.page);
    // Swap label text
    ...
    // Add small badge on the right
    if (p.badge && !node.querySelector('.gw-nav-badge')) {
      const b = document.createElement('span');
      b.className = 'gw-nav-badge';
      b.textContent = p.badge;
      b.style.cssText = 'margin-left:auto;background:rgba(58,194,255,.18);color:#3ac2ff;padding:2px 6px;border-radius:5px;font-size:9px;font-weight:800;letter-spacing:.3px;align-self:center';
      node.appendChild(b);
    }
    // Insert right after Рынки
    markets.parentNode.insertBefore(node, markets.nextSibling);
  }
  return true;
}

// Global capture-phase click handler: covers our injected items AND
// hides our overlay when the user clicks another nav-item.
function gwBindRouter() {
  if (window.__gwRouterBound) return;
  window.__gwRouterBound = true;
  document.addEventListener('click', (e) => {
    const navItem = e.target.closest('aside.sidebar .nav-item[data-page]');
    if (!navItem) return;
    const page = navItem.dataset.page;
    if (page === 'predict' || page === 'xstocks') {
      // Stop Cursor's default router — we render this ourselves
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation?.();
      gwShowExtraPage(page);
    } else {
      // User clicked some other nav-item → hide our overlay so Cursor's
      // page can show through
      gwHideExtraPage();
    }
  }, true);
  // Also hide overlay if hash changes to something else
  window.addEventListener('hashchange', () => {
    const h = (location.hash || '').replace(/^#/, '').split('?')[0];
    if (h !== 'predict' && h !== 'xstocks') gwHideExtraPage();
    else gwShowExtraPage(h);
  });
}

function gwSetupExtraNavObserver() {
  gwBindRouter();
  if (gwInjectExtraNavItems()) {
    // If URL already says #predict/#xstocks, render it
    const h = (location.hash || '').replace(/^#/, '').split('?')[0];
    if (h === 'predict' || h === 'xstocks') gwShowExtraPage(h);
    return;
  }
  let tries = 0;
  const interval = setInterval(() => {
    tries++;
    if (gwInjectExtraNavItems() || tries >= 30) {
...
window.gwShowExtraPage = gwShowExtraPage;
window.gwHideExtraPage = gwHideExtraPage;
